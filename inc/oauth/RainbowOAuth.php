...
class RainbowOAuth
{
    ...
    public function getAuthUrl($callbackUrl = null, $state = null, $scope = null)
    {
        ...
        {
            throw new InvalidArgumentException('callbackUrl 不能為空');
        }
        ...
        if (empty($url))
        {
            throw new InvalidArgumentException('彩虹聚合登入返回的跳轉地址為空');
        }
    ...
    public function getAccessToken($storeState = '', $code = null, $state = null)
    {
        ...
        {
            throw new InvalidArgumentException('state 驗證失敗');
        }

        $code = $code ?? ($_GET['code'] ?? '');
        if (empty($code))
        {
            throw new InvalidArgumentException('缺少回檔參數 code');
        }
        ...
        {
            throw new InvalidArgumentException('彩虹聚合登入返回 social_uid 為空');
        }
    ...
    public function getUserInfo($accessToken = null)
    {
        ...
        if (empty($this->openid))
        {
            throw new InvalidArgumentException('openid 為空，無法查詢使用者資訊');
        }
    ...
    private function request(array $query): array
    {
        ...
        {
            throw new InvalidArgumentException('彩虹聚合登入 AppID/AppKey 未設定');
        }
        ...
        if (!str_starts_with($apiBase, 'http://') && !str_starts_with($apiBase, 'https://')) {
            throw new InvalidArgumentException('介面位址格式不正確');
        }
        ...
        if (is_wp_error($resp))
        {
            throw new InvalidArgumentException('請求彩虹聚合登入失敗：' . $resp->get_error_message());
        }
        ...
        if (!is_array($data))
        {
            throw new InvalidArgumentException('彩虹聚合登入返回資料解析失敗');
        }

        $code = (int)($data['code'] ?? -1);
        if ($code !== 0)
        {
            $msg = (string)($data['msg'] ?? '請求失敗');
